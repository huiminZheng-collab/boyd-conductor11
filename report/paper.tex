...
conjecture:\\ a certified proof of the split-integral identity
and a structural theorem for the family $S_k$}
...
around the branch cut equals $b_{11}=L'(E_{11},0)$. We give a proof of (C3)
at the level of certified computation: the split integral is a regulator
integral along Samart's signed open chain $\tilde\gamma$; we close
$\tilde\gamma$ by a small-branch compensating arc $\beta_0$ into a closed
anti-invariant integral cycle $C'=\tilde\gamma+\beta_0$, and certify its
homology class as $2\gamma^-$ via the a-priori integrality of the period
ratio $\period(C')/w_{\mathrm{anti}}$ matched to $15$ digits. Combined with
the exact integral identity $\int_{\beta_0}\eta=\int_{\tilde\gamma}\eta$ and
the theorems of Bloch, Brunault and Bertin, this yields
$I_{\mathrm{split}}=b_{11}$, in agreement with the $149$-digit numerical
value. We also prove a structural theorem for the whole family
$S_k=y^2+(x^2+kx+1)y+x^3$: the dividing line is the torus-intersection
structure, not the sign of $k$; we confirm numerically (60 digits, with
PARI/GP cross-checks) new Boyd-type identities for $k=2,3,-4,-5,-6$, and we
adjudicate negatively the conductor-$53$ analogue suggested by Samart.
\end{abstract}
...
\begin{enumerate}
  \item \textbf{Certified proof of (C3).} We prove
  ...
  $C'=\tilde\gamma+\beta_0$ on $E$ whose homology class is certified to be
  $2\gamma^-$, where $\gamma^-$ generates $H_1(E,\Z)^-$; the regulator side
...
mpmath at $60$--$300$ digits.

The prototype of the whole story is Smyth's 1981 formula
\[
...
isogeny class (LMFDB \texttt{11.a1--a3}), containing
\[
...
Deninger conjectured, and Rodr\'iguez Villegas explained, that for
...
conductor $14$; Rogers--Zudilin in conductor $15$; Lal\'in--Samart--Zudilin
in conductor $21$; Brunault in conductor $11$).
...
\emph{BMZ formula}: for Siegel units
...
digits). The conductors, confirmed by PARI \texttt{ellfromeqn} and
...
\begin{lemma}[Closed chain lemma]\label{lem:closed}
Let $\alpha_2$ be the small-branch inner arc $\theta:c\to-c$ (endpoints
...
\]
By Bloch's theorem in its original normalisation
$r(\{f,g\})=\frac{1}{2\pi}\int_\gamma\eta(f,g)$ ($\gamma$ generating
$H_1(E,\Z)^-$) one has $\pi\,r(\{f,g\})=D_E((f)\diamond(g))$, i.e.\
\[
...
arithmetic, $300$-bit working precision; output
...
yielding Rodr\'iguez Villegas--type rational relations between two different
polynomial models of the same curve.
...
exotiques},
J. Reine Angew. Math. \textbf{569} (2004).
...
elliptique},
th\`ese, Universit\'e Paris 7 / ENS Lyon, 2005.

\bibitem{Samart2023}
D.~Samart,
\emph{Mahler measures of a family of non-tempered polynomials and Boyd's
conjectures}, arXiv:2301.05390, 2023.
