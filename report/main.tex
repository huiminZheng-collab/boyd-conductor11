...
因子 $-2$ 经文献核对为 Bloch 定理的内在因子。证明链基本闭环
（\S\ref{sec:regulator}）。
\end{abstract}
...
证明纲要与第三波 regulator 常数计算（\S\ref{sec:regulator}）。
...
缺口不在 regulator 计算，而在于 \textbf{Boyd 的劈裂积分链与 $H_1(E,\ZZ)^-$ 生成元
的等同}——这正是我们第二波绕数 $n=1$（周期配对，60 位 + PARI 交叉验证）所锁定的那一环。

\textbf{结论}：\eqref{C3} 的全部成分均已就位，且每一环要么是已证定理
（tempered、modular units、Bloch Thm、Bertin Thm 6、Brunault (3.151)/(3.210)），
要么已被高精度数值 + 精确代数双重锁定（绕数 1、除子 \eqref{eq:divs}、
...
\item \textbf{证明链基本闭环}（\S\ref{sec:regulator}）：除子与金刚石积精确算出，
$D_E(P)=\frac{2\pi}{5}\bN$（40 位，精确吻合 Brunault (3.151)），exotic
$D_E(2P)=\frac32 D_E(P)$（Bertin 已证），合成 $-\frac52 D_E(P)=-\pi\bN$ 对
$\int\eta=2\pi\bN$；因子 $-2$ 经文献核对为 Bloch 定理的内在因子
（$\pi r=D_E(\diamond)$）乘定向符号。更有 Brunault (3.210) 已证
$|r_\gamma\{x,y\}|=\bN$——\eqref{C3} 的 regulator 一侧本来就是定理，缺的
``劈裂链 $=H_1(E,\ZZ)^-$ 生成元''一环由绕数 $n=1$ 锁定：\eqref{C3} 从``开放猜想''
降级为``已证定理的组装 + 书写''。
