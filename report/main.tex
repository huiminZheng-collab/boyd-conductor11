...
$F_{\mathrm{total}}=-2f_{11}$ 由 $251$ 系数精确 $q$ 展开证明）；Bloch/Bertin/字典退出
...
\begin{proof}
全部步骤为精确有理/$q$ 展开计算或已存档（\texttt{code/siegel\_anchor\_step1}--\texttt{11}，
\texttt{notes/attack16-siegel-anchor.txt}）。
\emph{(1) 尖点与除子}：在附属于 $f_{11}$ 的模参数化 $X_1(11)\to E$（无穷尖点
$\mapsto O$）下，有理尖点 $k/11\mapsto m_kA$，$(m_1,\dots,m_5)=(0,2,1,4,3)$，与
Brunault 的尖点表（(3.152)--(3.153)）一致；尖点 $i\infty$ 与 $1/11$ 在 $X_1(11)$ 上
重合（因 $\bigl(\begin{smallmatrix}1&0\\11&1\end{smallmatrix}\bigr)\in\Gamma_1(11)$）。
...
\emph{(2) Siegel 表示}：记 $G_a:=\prod_{b\bmod 11}g_{a,b}$；用 Kubert--Lang 尖点阶
...
（其周期先验为 $w_{\mathrm{anti}}$ 的非零整数倍，$60$ 位等于 $w_{\mathrm{anti}}$），
故即引理 \ref{lem:antiinv} 的类 $\pm(a-2b)$。连分数把它分解为七个 Manin 符号：
\[
\begin{aligned}
...
分圆系数域上的精确 $q$ 展开算术（$251$ 个系数，远超 $M_2(\Gamma_0(11))$ 的 Sturm
界 $2$）证明总权 $2$ 形式恰为
\[
F_{\mathrm{total}}=-2\,f_{11}\ \in\ M_2(\Gamma_0(11)):
\]
两个符号（$\bigl[\begin{smallmatrix}1&0\\3&1\end{smallmatrix}\bigr]$、
$\bigl[\begin{smallmatrix}1&2\\1&3\end{smallmatrix}\bigr]$）各贡献 $-f_{11}$，其余五个
恒为零。因此 $\Lambda$ 和等于 $-2\Lambda(f_{11},0)=-2\bN$——由
...
曲线上三个\textbf{独立} tempered symbol 考察此事：Weierstrass symbol、我们的 $S_0$
...
函数的精确恒等式（Kubert--Lang 阶的精确尖点除子匹配；常数为单位根，算到 $70$ 位）；
$\gamma^-$ 的 Manin 符号分解精确（连分数）；权 $2$ 形式恒等式
$F_{\mathrm{total}}=-2f_{11}$ 由精确 $q$ 展开算术证到 $251$ 个系数、远超 Sturm 界 $2$
（\texttt{siegel\_anchor\_step6/7/9}，存档 \texttt{notes/attack16-siegel-anchor.txt}）；
尖点-扭点对应与闭链本原性对其先验整数值 $60$ 位确认。
\end{enumerate}
