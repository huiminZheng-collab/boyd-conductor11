...
因子 $-2$ 经文献核对为 Bloch 定理的内在因子；
(vii) 第四波：$S_k$ 族结构定理 + conductor 53 矛盾裁决；
...
$-1$ & $53$ & $-1$ & $1/3$ & 比值 $0.7392026\ldots$ & \textbf{无有理关系}（结构解释，见下）\\
...
$-4$ & $37$ & $-1$ & 无（相切） & $m=\frac72\,b_{37}$ & \textbf{新恒等式}（预测后验证），25 位 \\
$-5$ & $359$ & $-1$ & 无 & $m=\frac14\,b_{359}$ & \textbf{新恒等式}（预测后验证），25 位 \\
$-6$ & $997$ & $-1$ & 无 & $m=\frac18\,b_{997}$ & \textbf{新恒等式}（预测后验证），25 位 \\
\hline
\end{tabular}
\end{center}
...
本次意外收获。
\item \textbf{$k=-1$ 矛盾已解决（第四波，\texttt{code/k53\_attack.py} + \texttt{k53.gp}）}：
Samart 2023 \S4 关于 conductor 53 的``类似猜想''只有一句话（无公式、无精度、无引用）。
我们证明它\textbf{不可能成立}：
\begin{enumerate}[nosep]
\item \textbf{拓扑障碍}：$k=-1$ 时 $|x|=1$ 上的 torus 交点为 $\theta=0$（两根 $e^{\pm2\pi i/3}$，
分支在此\textbf{交换}）与 $\theta=\pm\pi/3$（$y=\pm1$）。枚举所有以交点为断点、big/small
分支组合的\textbf{闭反不变链}：唯一解 $(1,1,1,1)$ 的周期为 $0$——\textbf{环面上不存在非平凡的
反不变闭链}，$H_1(E,\ZZ)^-$ 生成元无法在 torus 上实现，Boyd 型环面积分对象根本不存在。
（连续根沿圆走\textbf{两圈}才闭合，且整圈周期也是 $0$。）
\item \textbf{代数障碍}：53.a1 扭子群\textbf{平凡}、秩 1，$(0,0)$ 就是 Mordell--Weil
\textbf{生成元}（PARI \texttt{ellorder}$=0$）——$x,y$ \textbf{不是} modular units，
Beilinson--Brunault 机制不适用。
\item 数值判决：半环 $L_1$（连续根一圈，开链）周期与 $w_{\mathrm{anti}}$ 之比
$0.5492906\ldots$ 不在周期格中，regulator 积分与 $2\pi b_{53}$ 之比 $0.7392026\ldots$
无理（PSLQ 阴性）。\textbf{结论：Samart 的 conductor 53 记述极可能是低精度数值假阳性。}
\end{enumerate}
\item \textbf{扭点对照表}（\texttt{code/kfamily\_torsion.gp}）揭示真正的分野：
$k=0$ 扭子群 $\ZZ/5$、$\operatorname{ord}(0,0)=5$；$k=1$ 扭子群 $\ZZ/4$、
$\operatorname{ord}(0,0)=4$；$k=-3,-2,-1,2,3$ 扭子群平凡、$(0,0)$ 无限阶。
\item \textbf{修正后的结构定理（第四波，预测 + 验证）}：分野不是 $k$ 的符号，而是
\textbf{torus 交点结构}：
\begin{enumerate}[nosep]
\item $|k|\ge2$（无真正 torus 交点；$k=\pm4,\pm2$ 处相切退化）：标准 Deninger 机制直接给出
$m(S_k)=r_k\,|L'(E_k,0)|$——$k=2,3$ 已确认，$k=-4,-5,-6$ 为\textbf{先预测后命中}：
$m(S_{-4})=\frac72|L'(E_{37},0)|$、$m(S_{-5})=\frac14|L'(E_{359},0)|$、
$m(S_{-6})=\frac18|L'(E_{997},0)|$（25 位精确）。注意 $S_{-4}$ 与 $S_2$ 同为 conductor 37，
给出同一曲线的 Rodriguez-Villegas 型关系。
\item $-4<k<2$（真正 torus 交点，$m$ 本身失效，须用劈裂积分）：恒等式成立\textbf{当且仅当}
曲线有扭点使 $x,y$ 成为 modular units——全族中仅 $k=0$（$\ZZ/5$，$X_1(11)$）与
$k=1$（$\ZZ/4$）两例；$k=-1,-2,-3$ 扭子群平凡，无解。
\item 这与 Samart 的观察 $K\cap\RR=[-4,2]$（环面相交参数区间）精确吻合。
\end{enumerate}
\end{itemize}
...
S5 & $5D_E(P)-5D_E(2P)=-\pi\bN$ vs $\int\eta=2\pi\bN$；$-2$ = Bloch 内在因子（\S\ref{sec:regulator}） & 闭合 \\
\hline
\end{tabular}
\end{center}
...
\textbf{regulator 的直接锚定（第八波重修；取代旧的``因子 $-2$''读法）}：
Brunault 论文 (3.210)/(3.211) 引 Bertin [10, Thm 6 与 Cor.~6.1] 已直接给出
\[
|r_\gamma\{x,y\}|=\frac{5}{2\pi}D_E(P)=\frac{11}{4\pi^2}L(E,2)=\bN,
\qquad r_\gamma\{f,g\}:=\frac1{2\pi}\int_\gamma\eta(f,g),
\]
即 $\int_{\gamma^-}\eta=\pm2\pi\bN$。\textbf{regulator 一侧的等式本来就是
Bertin--Brunault 的已证定理}（他证 \eqref{C1} 时的中间结果），证明走这条路，
完全不需要金刚石积的归一化约定。exotic relation $D_E(2P)=\frac32D_E(P)$
由 Bertin（CRM Proc. Lecture Notes 36, 2004）证明（\textbf{更正}：此前误指
J.~Reine Angew. Math. 569——该文中此关系仍是猜想；zbMATH 书评与
Touafek--Kerada、Mellit 三处佐证 CRM 版才是证明出处）。

\textbf{归一化问题（第八波定论）}：Bloch 定理的正确陈述是因子 $\mathbf{1}$：
$r(\{x,y\})[\gamma]=D^E((x)\diamond(y))$、$r[\gamma]=\int_\gamma\eta$
（Lal\'in--Ramamonjisoa Thm.~6 原文；LSZ cond21 同）。判定依据：L--R 的
conductor-17 链中实闭链的类是 $\gamma$ 的未知整数倍 $C$，因子 1 给出
$C=1$（其已发表 eq.~(20)，我们 60 位复现），因子 2 则迫使
$C=1/2\notin\ZZ$。旧稿``$\pi r=D_E(\diamond)$（因子 2，Touafek 转述）''与此
不相容，已弃用。遗留问题（如实记录，不影响证明）：按修正除子展开的
$D_E(\diamond)=\pm\frac52D_E(P)=\pm\pi\bN$，恰为 Bloch+Bertin 所需值
$\pm5D_E(P)=\pm2\pi\bN$ 的一半——这个因子 2 住在 $\ZZ[E]^-$ 符号约定里
（除子、$D_E$ 级数、闭链均已三重验证排除嫌疑），论文中如实标注。

Samart 2023 仍将 \eqref{C3} 列为开放猜想，
...
\textbf{同调类（认证）}。属 1、$\Delta<0$：$H_1(E,\ZZ)^-=\ZZ\gamma^-$，周期配对单射，
$\mathrm{period}(\gamma^-)=\pm w_{\mathrm{anti}}$，故
...
符号=定向约定）。\textbf{更正}：第二波的``$\tilde\gamma$
本身是生成元（$n=1$）''不准确——$\tilde\gamma$ 是开链；正确的闭链 $C'$ 绕数为
\textbf{2}。
...
再由 Bloch（$\pi r=D_E(\diamond)$）+ Brunault (3.151)（$D_E(P)=\frac{2\pi}{5}\bN$）
+ Bertin exotic（$D_E(2P)=\frac32D_E(P)$）+ 金刚石积 $5(A)-5(2A)$（\S\ref{sec:regulator}）：
$\int_{\gamma^-}\eta=\pm2\pi\bN$。合成：
\[
\int_{\tilde\gamma}\eta=\frac12\int_{C'}\eta=\pm 2\pi\bN,
\]
符号由一次数值评估（$+2\pi\bN$，$\bN>0$）钉死。由结构定理
$I_{\mathrm{split}}=\frac{1}{2\pi}\int_{\tilde\gamma}\eta$（第一波，已证）：
\[
\boxed{\,I_{\mathrm{split}}=\bN\,}\qquad\text{\eqref{C3} 证毕}.
\]
...
\subsection{k=1（conductor 17）：同一方法的第一次再应用（第七波）}\label{sec:proofk1}
...
符号由数值（$-2\pi b_{17}$）钉死，结构恒等式给出
$\boxed{\tilde n(1)=b_{17}}$（Samart conductor-17 类比猜想，证毕）。
\item \textbf{归一化备注}：k=0 证明引 Bloch--Brunault 的 $r_\gamma$ 归一化
（$\int\eta=\pm2D_E$），k=1 引 Lal\'in 归一化（因子 1）；两者只是 $r_\gamma$ 与
$D^E$ 定义中的常规因子之差，各自被该曲线上的已发表公式钉死
（Brunault (3.210)--(3.211)；L--R + Zudilin）。
\end{itemize}
...
Bloch + Brunault (3.151)/(3.210) + Bertin exotic（均为定理），得
$\int_{\tilde\gamma}\eta=2\pi\bN$ 为\textbf{严格等式}，$I_{\mathrm{split}}=\bN$ 证毕。
\item 族结果（第四波完成）：$\tilde n(k)$ 表 + \textbf{结构定理}——分野是 torus 交点
结构而非 $k$ 的符号：$|k|\ge2$ 时 $m(S_k)=r_k|L'(E_k,0)|$（$k=2,3$ 确认，$k=-4,-5,-6$
\textbf{先预测后命中}，$r=2,1,\frac72,\frac14,\frac18$）；$-4<k<2$ 时恒等式成立当且仅当
有扭点使 $x,y$ 成 modular units，全族仅 $k=0$（$\ZZ/5$）与 $k=1$（$\ZZ/4$）。
\textbf{conductor 53 矛盾解决}：环面上不存在非平凡反不变闭链（枚举唯一解周期为 0）
+ 53.a1 扭平凡、$(0,0)$ 为 MW 生成元（非 modular units）——Samart 的 53 记述极可能是
低精度假阳性（\S\ref{sec:family}）。
\item \textbf{conductor 17 证毕（第七波，\S\ref{sec:proofk1}）}：同一方法再应用于
$k=1$——链结构逐字平行（$c=2\pi/3$、跳跃值 $y=\pm i$ 精确）、
$\mathrm{class}(C')=\pm2\gamma^-$ Arb 铁证、regulator 侧由 Lal\'in--Ramamonjisoa
已发表定理闭环——Samart 的 $\tilde n(1)=b_{17}$ 类比猜想成为定理。
\end{enumerate}
