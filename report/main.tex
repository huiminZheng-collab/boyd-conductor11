...
精度锁定，$D_E(P)=\frac{2\pi}{5}b_{11}$（40 位），证明链除一个归一化约定因子外
基本闭环（\S\ref{sec:regulator}）。
\end{abstract}
...
S4 & Beilinson--Brunault 定理 + Brunault (3.151) & \\
   & $L(E,2)=\frac{10\pi}{11}D_E(P)$，系数 40 位复核 & 已核（数值） \\
S5 & $5D_E(P)-5D_E(2P)=-\pi\bN$ vs $\int\eta=2\pi\bN$；$-2$ 为约定因子（\S\ref{sec:regulator}） & 数值闭合 \\
\hline
\end{tabular}
\end{center}
...
加链定向的\textbf{约定因子}，文献精确形式待核（S5 行）。

\textbf{结论}：除这个纯约定因子外，\eqref{C3} 证明链的\textbf{全部成分均已就位}，
且每一环要么是已证定理（tempered、modular units、Bloch Thm 6、Bertin exotic、
Brunault Thm 8），要么已被高精度数值 + 精确代数双重锁定
（绕数 1、除子 \eqref{eq:divs}、金刚石积 \eqref{eq:diamond}、$D_E$ 值）。
\eqref{C3} 由此从``开放数值猜想''降级为``已证定理的组装 + 书写级工作''。
...
$D_E(P)=\frac{2\pi}{5}\bN$（40 位，钉死 Brunault (3.151) 系数），exotic
$D_E(2P)=\frac32 D_E(P)$，合成 $-\frac52 D_E(P)=-\pi\bN$ 对 $\int\eta=2\pi\bN$
——仅剩一个 $D_E$ 归一化的约定因子 $-2$ 待对文献；\eqref{C3} 从``开放猜想''
降级为``已证定理的组装 + 书写''。
