...
(vii) 第四波：$S_k$ 族结构二分（数值确立的命题）+ conductor 53 否定裁决；
...
$0$ & $11$ & $+1$ & $1/2$ & $\tilde n=-b_{11}$ & \eqref{C3}，366 位 \\
...
本次意外收获。
\item \textbf{$k=-1$ 矛盾已解决（第四波，\texttt{code/k53\_attack.py} + \texttt{k53.gp}；第十波精确化 \texttt{code/k53\_smith.py}）}：
...
\item \textbf{修正后的结构二分（第四波，预测 + 验证；数值确立的命题）}：分野不是 $k$ 的符号，而是
\textbf{torus 交点结构}：
\begin{enumerate}[nosep]
\item $|k|\ge2$（无真正 torus 交点；$k=\pm4,\pm2$ 处相切退化）：标准 Deninger 机制预期直接适用，
$m(S_k)=r_k\,|L'(E_k,0)|$（$r_k$ 为小有理数）在所有已算情形获数值确认——$k=2,3$ 已确认，
$k=-4,-5,-6$ 为\textbf{先预测后命中}：
...
\item $-4<k<2$（真正 torus 交点，$m$ 本身失效，须用劈裂积分）：扭转二分与数值证据精确吻合
——Boyd 型恒等式\textbf{证于}两个有扭点使 $x,y$ 成为 modular units 的情形：
$k=0$（$\ZZ/5$，$X_1(11)$）与 $k=1$（$\ZZ/4$）；$k=-1,-2,-3$ 扭子群平凡，PSLQ 搜索
无有理关系（这三条的 torus 交点含 $\theta=0$ 处的分支交换：恰是环面与曲线相交但无
modular units 的情形）。
...
\textbf{regulator 的锚定（第十波重修；外部审稿 BLOCKER 修复）}：一个必须挑明的识别——
Brunault (3.210) 的 symbol $\{x,y\}$ \textbf{不是}我们的 symbol，而是 $E:y^2+y=x^3-x^2$ 的
...
而我们的 $\diamond_{S_0}=6(O)-5(A)+5(2A)$，$D_E=+\frac52D_E(P)$——\textbf{比率恰为 $-1$}
（非零，因 $D_E(P)=\frac{2\pi}{5}\bN\neq0$）。两个 symbol 的 tame symbol 全为 $\pm1$
（精确局部展开，\texttt{code/bertin\_diamond.py}：$\{x,y\}$ 在 $A,2A,3A,O$ 处
$T\in\{\pm1\}$；$\{x_W,y_W\}$ 有 $T_A=-1$、$T_O=+1$），故都在 Bloch 定理的适用域内。
新锚定逻辑：Bloch 定理在\textbf{任何}归一化下都把 tempered symbol 的 regulator 表为
$D_E(\diamond)$ 乘以一个只依赖 $(E,\gamma^-)$ 与约定、\textbf{不依赖 symbol} 的常数，
故积分比被迫等于 $\diamond$ 值比 $=-1$。而 Brunault (3.210)（引 Bertin [10]
（= J.~Reine Angew. Math. \textbf{569} (2004)，Thm 6 与 Cor.~6.1）；归一化
$r_\gamma\{f,g\}:=\frac1{2\pi}\int_\gamma\eta$，(3.211)）给出
\[
...
直接数值积分 $\eta(x_W,y_W)$，Richardson 外推到 9 位得 $-2\pi\bN$
（\texttt{code/bertin\_diamond.gp}）。由比率即得
\[
\int_{\gamma^-}\eta(x,y)=\pm2\pi\bN.
\]
\textbf{regulator 一侧由此成为已证定理}；旧稿``$K_2$ 秩 1 + 认证积分钉 $\lambda=1$''的
循环论证已删除。符号取决于定向（Brunault Remarque~20：$D_E$ 依赖 $E(\RR)$ 的定向，
...
$\Re\tau=0$；我们预期因子源于 Bloch 证明中单分支格的反不变闭链对基本域边界的
归一化，但未做机理级证明。由于 \eqref{C3} 的证明只用比率（常数消去），此问题不影响
...
\textbf{同调不变性引理}：$\int_{C'}\eta(x,y)$ 只依赖于 $C'$ 在 $H_1(E,\ZZ)^-$ 中的同调类。
事实上，由 $\{x,y\}\in K_2(E)\otimes\QQ$（tempered，\S\ref{sec:outline} 表 S2），
$\eta(x,y)$ 在 $\operatorname{div}(x),\operatorname{div}(y)$ 支撑处的留数为扭，故与 $\eta$
的配对下降到同调；链 $C'$ 避开
...
\textbf{两项实现披露（注）}：(i) 存档输出（如 \texttt{notes/attack10-interval.txt}）中显示的
...
\item 族结果（第四波完成）：$\tilde n(k)$ 表 + \textbf{结构二分}（数值确立的命题）——分野是
torus 交点结构而非 $k$ 的符号：$|k|\ge2$ 时 Deninger 机制预期适用、
$m(S_k)=r_k|L'(E_k,0)|$ 获数值确认（$k=2,3$ 确认，$k=-4,-5,-6$
\textbf{先预测后命中}，$r=2,1,\frac72,\frac14,\frac18$）；$-4<k<2$ 时扭转二分与证据
精确吻合：恒等式证于 $k=0$（$\ZZ/5$）与 $k=1$（$\ZZ/4$），$k=-1,-2,-3$ PSLQ 阴性。
\textbf{conductor 53 否定裁决}：环面上不存在非平凡反不变闭链（边界矩阵核恰一维，
精确有理线性代数；生成元 $(1,1,1,1)$ 的周期由 $1/u_++1/u_-\equiv0$ 恒等于 0）
