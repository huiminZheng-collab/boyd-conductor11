...
\textbf{除子代数}（三次模型上的射影计算，Abel 主除子检验通过）：
...
\textbf{闭环}：代入 \eqref{eq:diamond} 得
\[
...
\end{equation}
\textbf{因子 $-2$ 的文献核对（已解决）}：这不是误差，而是 Bloch 定理的内在因子。
按 Bloch 定理的原始形式（如 Touafek 2008 Thm 1 的转述）：约定
$r(\{f,g\})=\frac{1}{2\pi}\int_\gamma\eta(f,g)$（$\gamma$ 生成 $H_1(E,\ZZ)^-$），则
\[
\pi\,r(\{f,g\})=D_E\bigl((f)\diamond(g)\bigr),\qquad\text{即}\qquad
\int_\gamma\eta(f,g)=\pm2\,D_E\bigl((f)\diamond(g)\bigr),
\]
符号取决于 $\gamma$ 定向（$H_1(E,\ZZ)^-$ 的两个生成元差符号，故文献只写 $|r|$；
Brunault Remarque 20 也指出 $D_E$ 依赖 $E(\RR)$ 定向、只定到符号）。
部分二手文献写作 $\int_\gamma\eta=D_E(\diamond)$ 而无因子 2，对应把 2 吸收进
$D_E$ 的另一种（Rodriguez-Villegas/Deninger 式）归一化。\textbf{我们的 $-2$ 与 Bloch
级数定义下的定理完全一致。}

\textbf{更强的事实}：Brunault 论文 (3.210)/(3.211) 引 Bertin [10, Thm 6] 已直接给出
\[
|r_\gamma\{x,y\}|=\frac{5}{2\pi}D_E(P)=\frac{11}{4\pi^2}L(E,2)=\bN.
\]
也就是说，\textbf{regulator 一侧的等式本来就是 Brunault 的已证定理}（他证 \eqref{C1}
时的中间结果），exotic relation $D_E(2P)=\frac32D_E(P)$ 亦由 Bertin
（J. Reine Angew. Math. 569, 2004）证明。Samart 2023 仍将 \eqref{C3} 列为开放猜想，
...
金刚石积 \eqref{eq:diamond}、$D_E$ 值）。\eqref{C3} 由此从``开放数值猜想''
降级为``已证定理的组装 + 书写级工作''。
