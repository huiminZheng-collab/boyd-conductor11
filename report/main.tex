...
正确归一化是因子 $1$，k=0 的证明改锚 Bertin--Brunault 已证定理；
...
S5 & regulator 常数：Bertin--Brunault (3.210)/(3.211) 直接给出 $\int_{\gamma^-}\eta=\pm2\pi\bN$（已证定理，\S\ref{sec:regulator}，第八波重修后不再依赖金刚石积归一化） & 闭合 \\
\hline
\end{tabular}
\end{center}
...
再由 Bertin--Brunault (3.210)/(3.211)（$|r_{\gamma^-}\{x,y\}|=\frac{5}{2\pi}D_E(P)=\bN$，
$r=\frac1{2\pi}\int\eta$）+ Brunault (3.151)（$D_E(P)=\frac{2\pi}{5}\bN$）
+ Bertin exotic（$D_E(2P)=\frac32D_E(P)$）（均为定理，\S\ref{sec:regulator} 第八波重修）：
...
（$\int_{\gamma^-}\eta=\pm D^E(\diamond)$，因子 1）经 L--R 的 $C=1$ 整数性论证
确证为 Bloch 定理的正确形式；k=0 的证明不用 $\diamond$-形式，直接锚定
