...
正确归一化在 conductor-17 曲线上是因子 $1$，k=0 的证明改锚 Bloch 因子-1 定理的
余商生成元读法（第十二波：指数引理 \ref{lem:coinvariant}，$\Delta<0$ 时指数 $2$）；
...
S5 & regulator 常数：余商生成元上 Bloch 因子-1 锚定 $\int_{[b]}\eta=\pm\pi\bN$，经指数引理（引理 \ref{lem:coinvariant}，$\Delta<0$，指数 $2$）翻倍得 $\int_{\gamma^-}\eta=\pm2\pi\bN$（已证定理，\S\ref{sec:regulator}） & 闭合 \\
\hline
\end{tabular}
\end{center}
...
\textbf{regulator 的锚定（第十波重修；第十二波：单一归一化 + 指数引理）}：$\{x,y\}$ 沿
$\gamma^-$ 的 regulator 评估全程使用\textbf{单一固定归一化}下的 Bloch 定理，外加一条
指数引理，比较 $\Delta<0$ 曲线上``$H_1(E,\ZZ)^-$''的两种标准含义。
...
（coinvariant quotient）$H_1(E,\ZZ)/H_1(E,\ZZ)^+$ 上：
\begin{enumerate}[nosep]
...
\paragraph{Bloch 定理}本文用 Lal\'in--Ramamonjisoa Thm.~6 表述的 Bloch 定理，归一化取
L--R Def.~5 eq.~(10)（与我们 $D_E$ 的级数实现逐字一致，两条曲线上均 60 位核对）：
对 $f,g\in\QQ(E)$、$\{f,g\}\in K_2(E)$，
\[
\int_{\bar\gamma}\eta(f,g)=D_E\big((f)\diamond(g)\big),\tag{B}\label{eq:bloch}
\]
其中 $\bar\gamma$ 是余商 $H_1(E,\ZZ)/H_1(E,\ZZ)^+$ 的生成元——regulator 天然定义在其上
的那个群。两端对 symbol 均 $\QQ$-线性，故 \eqref{eq:bloch} 从 $K_2(E)$ 延拓到
$K_2(E)\otimes\QQ$，即全部 tempered symbol。$\Delta>0$ 时余商与反不变生成元一致
（引理 \ref{lem:coinvariant}~(1)），\eqref{eq:bloch} 正是 L--R Thm.~6 在其 conductor-17
计算中所用的陈述——附录 \ref{sec:proofk1} 依赖的正是这个\textbf{已证实例}；$\Delta<0$
时两种读法差指数 $2$（引理 \ref{lem:coinvariant}~(2)），而余商读法 \eqref{eq:bloch}
在本曲线上由三个独立 tempered symbol 与 $\int_{[b]}\eta$ 的直接数值评估双重确认
（见下``因子 2——已解决''段）。

\paragraph{应用到我们的 symbol}symbol $\{x,y\}$ 是 tempered 的，$\diamond$ 值为
\[
D_E\big((x)\diamond(y)\big)=-5D_E(P)+5D_E(2P)=\tfrac52\,D_E(P)=\pi\,\bN\neq0
\]
（用 Bertin exotic relation 与 Brunault 的系数——Thm.~8 与 Cor.~101，eq.~(3.151)——
均见上文）。Bloch 定理 \eqref{eq:bloch} 给出 $\int_{[b]}\eta(x,y)=\pm\pi\bN$；本曲线
$\Delta=-11<0$，引理 \ref{lem:coinvariant}~(2) 把它翻倍：
\[
\int_{\gamma^-}\eta(x,y)=\pm2\pi\bN,\tag{A}\label{eq:anchor}
\]
符号取决于定向。\textbf{regulator 一侧由此成为单一归一化下的已证定理}；余下的缺口——
Boyd 劈裂积分链与闭链 $C'=2\gamma^-$ 的等同——由 \S\ref{sec:proofc3} 的认证闭合。
旧稿``$K_2$ 秩 1 + 认证积分钉 $\lambda=1$''的循环论证已删除。exotic relation $D_E(2P)=\frac32D_E(P)$
...
全部三个独立 tempered symbol 观测到的统一因子 $2$（见下段）。这不影响任一证明：
conductor-17 论证在自己的曲线上自洽，conductor-11 论证经 conductor-17 已证实例把余商值
锚定在 Bloch 定理上。
...
\]
前两个随细分以 $O(1/N)$ 收敛、Richardson 外推（$1/N$ 线性）与 $-\pi\bN$ 吻合到
$10^{-6}$，最后一个到 $10^{-22}$；且 $-2\int_b\eta=\int_{a-2b}\eta$ 以同精度成立。
特别地，因子-1 恒等式 \eqref{eq:bloch} 对 $\{x_W,y_W\}$ 在余商生成元上成立，其子群
生成元读数即加倍的 \eqref{eq:anchor}。副产品：
...
再由 \S\ref{sec:regulator} 的锚定：$\{x,y\}$ 的 $\diamond$ 值
$\frac52D_E(P)=\pi\bN$（Brunault (3.151) $D_E(P)=\frac{2\pi}{5}\bN$ + Bertin exotic
$D_E(2P)=\frac32D_E(P)$，均为定理），Bloch 因子-1 恒等式 \eqref{eq:bloch} 给出余商值
$\int_{[b]}\eta(x,y)=\pm\pi\bN$，引理 \ref{lem:coinvariant}~(2)（本曲线 $\Delta=-11<0$，
指数 $2$）把它翻倍，故 $\int_{\gamma^-}\eta(x,y)=\pm2\pi\bN$（\eqref{eq:anchor}）。合成：
\[
...
Euler 乘积精确成立。
\end{enumerate}
主定理证明中唯一的非机器步骤是 \S\ref{sec:regulator} 的 regulator 评估，它使用
\textbf{单一归一化}下的 Bloch 定理（Lal\'in--Ramamonjisoa Thm.~6 的表述），连同
Brunault Cor.~3.5.101（eq.~(3.151)）的两个 $D_E$ 值；其 symbol 字典（Thm.~118，
eq.~(3.210)--(3.211)）只作交叉验证（\S\ref{sec:regulator} 末``与 Brunault symbol 字典的
交叉验证''段）。
\end{theorem}
...
单一归一化下的 Bloch 定理 + Brunault (3.151) + Bertin exotic（均为定理；余商生成元上
因子-1 锚定 $\int_{[b]}\eta=\pm\pi\bN$，经指数引理（引理 \ref{lem:coinvariant}，
$\Delta<0$，指数 $2$）翻倍得 $\int_{\gamma^-}\eta=\pm2\pi\bN$，\S\ref{sec:regulator}），得
