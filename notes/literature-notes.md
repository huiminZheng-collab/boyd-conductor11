...
### 更正与严格化（2026-08-05 第五波）：绕数表述
- 上述"γ̃ = H_1(E,Z)⁻ 生成元（n=1）"**不准确**：γ̃ 是带边开链（端点 P=(i,e^{iπ/4}) 非扭，
  endpoint_torsion2.py 精确群律 20 倍无周期），其周期 = w_anti 只是启发证据。
- 正确闭链：C' = γ̃ + β_0（β_0 = 小分支补偿弧：内弧 P→P̄ + 外弧 −P→−P̄；注意内弧
  连接不了 −P→−P̄，必须走外侧——这是第五波的关键修正）。∂C'=0、c(C')=−C'。
- **认证**：period(C')/w_anti 先验整数，实测 1.9999999999999999（|差|=2.5e-16，
  60/80 位对照）⟹ class(C') = **2γ⁻**。
- ∫_{β_0}η = 2(J_1−J_2) = ∫_γ̃ η（精确积分代数）⟹ ∫_γ̃η = (1/2)∫_{C'}η
  = (1/2)·2·(±2πb_11)（Bloch+Brunault+Bertin）= ±2πb_11 **严格等式**。
- **(C3) 证毕（认证级别）**。完整证明：notes/proof-n1.md；数值：code/n1_certify.py，
  输出 notes/attack9-n1.txt。
